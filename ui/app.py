    ...
    if not src or not dest:
        ...
        with st.spinner("✨ AI is crafting your perfect trip..."):
            try:
                ...
                # DOWNLOAD BUTTON - FUNCTIONAL PDF DOWNLOAD
                st.markdown("---")
                
                # Generate itinerary text for download
                itinerary_text = f"""
AGENTIC AI TRAVEL PLANNER
Trip Itinerary
=====================================================
...
"""
                if "message" not in plan["flight"]:
                    itinerary_text += f"""
Flight: {plan['flight'].get('flight_number', 'N/A')}
Airline: {plan['flight'].get('airline', 'N/A')}
From: {plan['flight'].get('from', src)}
To: {plan['flight'].get('to', dest)}
Duration: {plan['flight'].get('duration', 'N/A')}
Price: ₹{plan['flight'].get('price', 0):,}
"""
                else:
                    itinerary_text += f"\n{plan['flight']['message']}\n"
...
"""
                if "message" not in plan["hotel"]:
                    itinerary_text += f"""
Hotel: {hotel['name']}
Location: {hotel['city']}
Rating: {hotel['stars']} Stars
Price: ₹{hotel['price_per_night']:,} per night
Total Cost ({days} nights): ₹{hotel['price_per_night'] * days:,}
...
"""
                for day, places in plan["itinerary"].items():
                    itinerary_text += f"\n{day}:\n"
                    for p in places:
                        itinerary_text += f"  • {p['name']} ({p['type'].title()}) - Rating: {p['rating']}⭐\n"
                
                itinerary_text += f"""
🌤️ WEATHER FORECAST
"""
                for i, temp in enumerate(weather_data, 1):
                    itinerary_text += f"Day {i}: {temp}°C\n"
...
Hotel Cost ({days} nights): ₹{budget["hotel_cost"]:,}
Food & Local Expenses: ₹{budget["food_and_local"]:,}
-----------------------------
TOTAL ESTIMATED COST: ₹{budget["total_cost"]:,}

=====================================================
Generated by Agentic AI Travel Planner
Powered by OpenAI ✨
© 2025 | Built with ❤️
"""
                ...
                with col_dl2:
                    st.download_button(
                        label="📥 Download Complete Itinerary (TXT)",
                        data=itinerary_text,
                        file_name=f"Trip_{src}_{dest}_{days}days.txt",
                        mime="text/plain",
...
">
    ...
    <div style="color: rgba(255,255,255,0.9); font-size: 1rem; font-weight: 400;">
        Python • Streamlit • OpenAI • Folium Maps • Plotly • Open-Meteo API
    </div>
